...
# Set format to add_annotations
rownames(deseq2_results) <- deseq2_results$gene_id
deseq2_results_annot <- add_annotations(object    = deseq2_results,
...
# Change rownames from vst to hgnc symbols and clean format
idx_vst      <- match(rownames(vst_counts), annotations$geneID)
new_rownames <- rownames(vst_counts)

valid_vst <- !is.na(idx_vst) &
             !is.na(annotations$symbol[idx_vst]) &
             annotations$symbol[idx_vst] != ""

new_rownames[valid_vst] <- annotations$symbol[idx_vst[valid_vst]]

vst_sym               <- vst_counts
rownames(vst_sym)     <- new_rownames
vst_sym               <- vst_sym[!duplicated(rownames(vst_sym)), ]

# Plot
heatmap_PA(
  expression_data = vst_sym,
  metadata        = sampledata,
  pa_data_annot   = pa_annot,
  ranked_genes    = ranked_genes,
  plot_genes      = c("all_genes"),
