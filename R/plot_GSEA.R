...
#'
#' Creates either a global GSEA plot, could be a faceted barplot depending on the number of unique
#' comparisons in the `Comparison` column. Allows customizing all previously hard-coded theme
#' parameters via a single `theme_params` list (option 2).
#'
...
#' @param theme_params Named list to override any of the following defaults:
#'   \describe{
#'     \item{side_label_size}{Size for side panel labels (default 35)}
#'     \item{panel_widths}{Numeric vector of length 6 for patchwork widths (default c(4,25,15,3,10,3))}
#'     \item{bar_col}{Bar/col border color (default "black")}  
#'     \item{bar_size}{Border size for bars (default 0.5 for barplot mode)}
...
#'     \item{axis_text_size_y}{Font size for y-axis text in barplot mode (default 50)}
#'     \item{tick_size}{Size for axis ticks (default 1.5)}
#'     \item{tick_length}{Length for axis ticks in cm (default 0.3)}
#'     \item{strip_text_size}{Font size for strip text in facets (default 50)}
#'     \item{panel_spacing_single}{Spacing for single-comparison facets in lines (default 4)}
#'     \item{panel_spacing_multi}{Spacing for multi-comparison facets in lines (default 0.6)}
#'   }
...
plot_GSEA <- function(
  ...
  params <- modifyList(defaults, theme_params)
  
  # Compute or assign logFDR
  ...
  if (length(unique(data[[Comparison]])) == 1) {
    ...
    plot_text_pathways <- ggplot() +
      annotate("text", label="Pathways", fontface="bold.italic",
               angle=90, size=params$side_label_size, x=0, y=0.5) + theme_void()
    ...
    plot_center <- ggplot(df, aes(x=.data$NES, y=.data$Geneset, fill=.data$logFDR)) +
      ...
      facet_grid(Collection~., scales="free_y", space="free_y") + theme_bw() +
      labs(x="NES", y="") +
      theme(
    ...
    plot_right <- ggplot(df, aes(y=.data$Geneset, x=1.5, label=.data$Collection)) +
      geom_text(aes(label=ifelse(duplicated(.data$Collection),"",.data$Collection)),
                hjust=0.5, size=text_size_collection, fontface="bold") +
      facet_grid(Collection~., scales="free_y", space="free", switch="y") +
      theme_void() + theme(
        strip.text.y=element_text(size=text_size_collection),
    ...
    final_plot <- ggplot(data, aes(x=.data[[Comparison]], y=.data[[axis_y]], fill=.data$logFDR)) +
      ...
      theme(
        axis.line.x=element_blank(), axis.line=element_line(size=0.5),
        axis.title.x=element_text(size=params$axis_title_size),
    ...
    if (!is.null(custom_labels)) final_plot <- final_plot + scale_x_discrete(labels=custom_labels)
  }
